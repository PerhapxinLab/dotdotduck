...
### `useSurface()`
Listen for and render Surfaces emitted by skills / webagent:
...
Under the hood `useSurface` subscribes to the `surface` event and exposes the most recent payload + helpers. See [08-surface-renderer](./08-surface-renderer.md) for the data shape.
...
Surface 的 portal host — pre-wires `useSurface()` and `PieceRenderer` into a chosen placement. React 用戶可控制 portal target。
