...
| `inline`  | 字幕條上方的短確認條                                   | 不會 — 用戶可以繼續看頁面 |
| `dock`    | 常駐側邊 panel（進度追蹤、任務清單）                    | 不會 — 用戶工作時一直在 |
...
1. 用戶在 palette 打 `/order-status` 按 Enter
...
如果一切都在 dddk 內部 — skill emit `PieceSurface`、host 用 `PieceRenderer` render、用戶按 button、host 呼叫 `skill.onSubmit(data)` — 你完全不用碰 envelope 格式。Bridge 是給 boundary 用的；boundary 之內，直接用 tree。
