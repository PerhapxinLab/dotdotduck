...
> Skill = 可註冊的「dotdotduck 行為單元」。四種類型涵蓋從純文字 prompt 到視覺導覽到 Surface（結構化 UI）。
...
### 4. SurfaceSkill — 開出一個結構化 UI

最適合「需要結構化輸入 / 顯示結構化結果」。回傳一個 `PieceSurface`（根節點 + 可選的資料模型）。dotdotduck 會 emit `surface` event，host 用 `PieceRenderer` 渲染。詳見 [08-surface-renderer](./08-surface-renderer.md)。
...
const orderStatus: SurfaceSkill = {
  ...
  onSubmit: async ({ orderId }, ctx) => {
    ...
    return {
      // 第二個 surface 顯示結果（多步表單）；回 undefined 就關閉
      root: {
        kind: 'Card',
        children: [
...
`build` 產第一個 surface，`onSubmit` 收用戶輸入後可以回新 surface（多步表單）或 `undefined`（結束）。
...
`SkillRegistry` 是 dotdotduck 內建的，host 透過 config 傳 skills：
...
`tools.surface()` 行為跟 `SurfaceSkill` 一樣 — emit `surface` event，等 host 渲染並回 `dddk.submitSurface(data)` / `dddk.cancelSurface()`。差別是 ScriptSkill 可以**中途**插一個臨時表單再繼續，不像 SurfaceSkill 整個 skill 就是一個 surface。詳見 [02-how-to-add-a-skill](./02-how-to-add-a-skill.md) 的「Mid-script Surface」段落。
...
dotdotduck 不附任何內建 skill。所有 skill 由企業 / 開發者自己寫，自己 register。
...
需要寫範例 skill 時，看 [10-cookbook-enterprise-skills.md](./10-cookbook-enterprise-skills.md)（cookbook 是 doc，不是 export 出去的程式碼）。
...
詳細範例見 [10-cookbook-enterprise-skills.md](./10-cookbook-enterprise-skills.md)。
