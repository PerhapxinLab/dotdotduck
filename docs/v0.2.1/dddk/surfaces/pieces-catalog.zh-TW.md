...
- **`Group`** — 純粹的 flex 容器，無邊框。`direction: 'vertical' | 'horizontal'` + `gap` + `align`。
- **`MediaCard`** — 圖 + 標題 + 描述 + meta 標籤，自己沒有邊框。`orientation: 'top' | 'left'` 切上圖下文 / 左圖右文。
- **`OptionGroup`** — keyboard-navigable 選項列表，每個 option 是 MediaCard 形狀。內建上下左右鍵 + Enter / Space + click，fires `ctx.trigger('choose', { value, index })`。`layout: 'row' | 'column'` + `columns` 控制版型。
- **`ChoiceList`** — 純文字版 `OptionGroup`，無圖、密度更高，confirm-with-options 用。

完整 API 跟範例見 [composition.md](./composition.md)。
...
需求：一行多個 colored badge，每個可帶 emoji + label，點下去發 `select` action 告訴 host 選了哪一個。
...
Catalog 是「dddk world 的 element」，Slot 是「逃生口」。建議：先用 Slot 開發，反覆用了再升級成 PieceDefinition。
